...
#ifndef ROLLOFFMONOFILE_TIMEMODEL_H
#define ROLLOFFMONOFILE_TIMEMODEL_H

#include "Point.h"

int TimeModel(string Pointpath, string path,
              double dz = 0.001, double max_face = 150, double min_face = 50,
              int lim = 10, int add_float = 2, int min_point_num = 10, int timegap = 5) {
    int num_time_name = 1;
    double day_min_z = 0;
    double day_max_z = 0;
    ...
    for (int i = 1; i < fileroot.size(); i++) {
        ...
        if (min_t > fileroot[i].time) min_t = fileroot[i].time;
        if (max_t < fileroot[i].time) max_t = fileroot[i].time;
    }
...
#endif //ROLLOFFMONOFILE_TIMEMODEL_H
